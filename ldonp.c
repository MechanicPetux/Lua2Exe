...
// The only change in this file is f_parser
// The reason to include whole file is "multiple definitions" errors
// The summarized point is linker only can include whole object file, and not required stuff
...
void f_parser (lua_State *L, void *ud) {
	...
	if (c == LUA_SIGNATURE[0]) {
		...
		luaO_pushfstring(L,
			"attempt to load a text chunk (disabled at compilation)");
		luaD_throw(L, LUA_ERRSYNTAX);
	}
